...
preferred routeとeffective routeを別の型付き値とし、選択根拠を観測可能にする。各routeは`CodexThread`または`NoExternalContinuity`のContextContinuity能力を広告し、effective route/provider/profile/versionとともにdispatch前のEffect／pending recordへ固定する。Codex Threadを継続できるrouteだけが記録済みexact Threadをresumeする。Hoshikage/Ollama等の非対応routeは現在入力とY2が選んだSemanticMemoryだけを受け取り、外部履歴継続を主張しない。Codex routeへ戻る場合は以前のexact Threadを再開できるが、別routeで行われたturnまでThreadに含まれるとは主張しない。設定変更は次のInteractionからだけ適用し、active turnをrebindしない。Provider間、local/remote間、または同一Provider内の自動fallbackは行わない。選択routeが利用不能またはdispatch不能なら、型付きterminal FailureまたはRecoveryを返す。`FallbackToConversation`はBehavior候補なしだけのDecisionであり、Provider失敗の代替にしない。
