...
Qualia Contextだけが非Home sessionの開始とHome復帰を所有する。Interaction Contextだけが入力受理、request idempotency、取消を所有する。両者のread-only viewを読むadmission Ruleは純粋であり、直接Stateを変えない。非Homeで別Qualiaを求めたときは`Busy`であり、待機列を暗黙に作らない。通常入力を現在Qualiaのcontrolとして扱うかはBehavior Policyが決める。HomeとWeb Cancelは常に優先する共通制御である。TTS再生中の音声Stop候補だけは、自己音声ループ防止の`StopSuppressionPolicy`を先に通り、抑止された候補からCancel Commandを作らない。
