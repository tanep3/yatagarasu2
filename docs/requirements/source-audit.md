...
| 05 §7 | 会話と記憶 | accepted | 所有、standing authorization、local既定auto-save、explicit memorize、LLM前のrecent/semantic retrieval、5目的、typed Presentation、初期有限fallbackをADR-017/019とREQ-MEM-001/REQ-OUT-001/REQ-CNV-001へ採用。MemoryはOwner deleteまで保持し、連続会話は延期 | AC-MEM-001–006, AC-OUT-001–004, AC-CNV-001–004 |
...
| `workspace/.codex/skills/recall`, `README.md:50–55`, `bin/recall-context.sh` | recall（過去記憶検索） | accepted | Codex Threadを外部継続文脈、Yatagarasu Conversation/Memoryを正本と分離。通常Conversationの既定取得をrecent 0 + semantic 3、設定可、自動保存ONとし、5目的、provenance、空結果、Thread reset/delete制約をADR-017/022・REQ-MEM-001へ正規化。保存engineは未決、MemoryはOwner deleteまで保持 | AC-MEM-001–006, AC-AGT-007 |
...
| `workspace/.codex/skills/tanechan-search`, `workspace/AGENTS.md:17,40–43` | tanechan-search（現在情報のURL検索） | accepted | Searchをallowlist、provenance、citation、Failure/no-results、内容分類別LLM transfer authorizationが別のCapabilityとしてREQ-NET-001/ADR-021へ正規化。managed Search実通信はY2の型付きTool/Portへ限定し、一般Skillのgrant内network能力とは分ける。具体allowlistは未決 | AC-NET-001–005 |
| `workspace/.codex/skills/tanechan-fetch`, `workspace/AGENTS.md:18` | tanechan-fetch（URL内容取得） | accepted | FetchをSearchと同じnetwork境界だが別CapabilityとしてREQ-NET-001/ADR-021へ正規化。managed Fetch実通信は型付きPortを通し、直接通信をprovenance付き成功にしない。取得本文はWorldStateを所有せず、retrieval許可とProvider転送許可を分離する | AC-NET-001–005 |
| `workspace/.codex/skills/skill-creator`, `README.md:56` | skill-creator（Codex Skill作成） | accepted with normalization | SkillCreatorを初期必須Codex capabilityとする。OwnerはSkill作成と初期grant構成を包括委任し、asset/grant/activation完了時に個別承認なしで有効化する。SkillはWorkspace外副作用をgrant範囲で使えるが自己拡大せず、外部資産は正式Y2 Behavior/State所有へ自動昇格しない | ADR-007/015/023, REQ-SKL-001, AC-SCP-003, AC-PRD-016/018 |
...
| `python/listend.py:351–352,1557–1565,1706–1717,2313–2332`, `docs/plan/livekit-wakeword-requirements.md:125` | 「考えるね」「待機します」の設定と再生位置 | accepted with normalization | ThinkingNoticeは文言/有効無効を設定でき、LLM dispatch直前のvoice-only通知として継承する。SBERT反射とWebでは出さず、FailureでもLLMを止めない。Y2の通常/明示Homeへ「待機します」を結合しない | AC-OPS-029 |
| `bin/memorize.sh`, `bin/recall-context.sh`, `external/SemanticMemory`, `README.md:40–47`, `docs/semanticmemory-ruri-v3-migration.md` | 記憶の保存/検索、Ruri v2/v3 migration/versioning | accepted with rejection | 保存/想起の所有、standing authorization、delete/retention境界、5目的とprovenanceを持つversion付き既定recall、enabled初期ConversationのLLM前retrieval、content class/方向別PolicyをREQ-MEM-001/REQ-DAT-001へ採用。Y2はY1 import job/API/Upgrade migrationを提供しない。同じ互換storeですでに可視なrecordだけをprovenance付きで扱う。具体schema/engineは未決 | AC-MEM-001–006, AC-DAT-001–007 |
| `README.md:115–145` | Codex/Claude/opencode実行engine | accepted with normalization | 初期Agent adapterはCodexのみ。複数推論能力の動的選択をREQ-PRD-004/ADR-011へ採用し、EngineはAdapter/Providerでありdomain Stateを所有しない。routeはconfigured choiceとContextContinuity能力をbindし、非対応ProviderへCodex Thread継続を偽装せず、Failure/Recoveryを返し自動fallbackしない | AC-PRD-009–011, AC-PRD-017/020, AC-AGT-001/008 |
...
| `README.md` WakeWord節, `python/listen_state.py`, `python/tests/test_listend_wake_flow.py`, `docs/plan/livekit-wakeword-test-results.md` | prompt guard、空session、最初の発話、自己音声、Wake実測 | accepted with later owner policy | 一wake一命令、Acoustic/Execution所有、「はい」の非transcript、実TTS自己ループ防止を採用。TTS中音声Stopは回答全文にStop語があれば意図的に抑止し、なければCancellationへ渡すOwner決定で正規化する。TC70 spikeとOwner採否を初期release gate、C210を独立profile gateとする。Web Homeは生存する。RTSP resetは必須方式にしない | AC-ACOU-001–007, AC-QPR-001–004 |
